...
import { config, electroneumMainnet, electroneumTestnet } from '@/utils/web3-config'; // Import chain objects
import { 
  MagnifyingGlass, 
  Star, 
  ArrowSquareOut,
  ...
  FunnelSimple,
  Download,
  ListChecks, // Removed unused FileText
  CircleNotch,
  Copy
} from 'phosphor-react';
...
export default function ReportsPage() {
  ...
  const fetchAllChainAudits = useCallback(
    async (currentChain: typeof chain, switchChainFn: typeof switchChain) => {
      setIsLoading(true);
      try {
        const allAudits: AuditReport[] = [];
        const BATCH_SIZE = 50;

        // Use a union type for chainData to match electroneumMainnet or electroneumTestnet
        for (const [chainKey, chainData] of Object.entries(CHAIN_CONFIG) as [ChainKey, typeof electroneumMainnet | typeof electroneumTestnet][]) {
          try {
            console.log(`Fetching from ${chainKey}...`);

            if (currentChain?.id !== chainData.id) {
              await switchChainFn({ chainId: chainData.id }); // 52014 | 5201420
            }

            const totalContracts = (await readContract(config, {
              address: CONTRACT_ADDRESSES[chainKey],
              abi: AUDIT_REGISTRY_ABI,
              functionName: 'getTotalContracts',
              chainId: chainData.id, // 52014 | 5201420
            })) as bigint;
            ...
            while (processed < Number(totalContracts)) {
              try {
                ...
                const auditBatch = (await readContract(config, {
                  ...
                  args: [processed, BATCH_SIZE],
                  chainId: chainData.id, // 52014 | 5201420
                })) as AuditBatchResponse;
    ...
    [setIsLoading, setReports]
  );

  useEffect(() => {
    fetchAllChainAudits(chain, switchChain);
  }, [chain, switchChain, fetchAllChainAudits]);
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="font-mono bg-gray-800/70 px-3 py-2 rounded-lg border border-gray-700/70 flex items-center justify-between">
                      ...
                      <button
                        onClick={() => navigator.clipboard.writeText(selectedReport.transactionHash)}
